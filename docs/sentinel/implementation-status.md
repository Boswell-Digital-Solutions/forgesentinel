...
| C — Correlation in shadow | partial | Watchdog findings adapt to source findings; shared-root accounting prevents double counting; Prime promotion explicit. Live source now exists (below, SNT-115) -- neither of its two detector finding_types is wired into an existing `CorrelationRule` or `promoteSingles()`, so today they validate into the evidence ledger without forming an incident; that is scoped, not a gap. |
