...
| C — Correlation in shadow | done (MVP) | Watchdog findings adapt to source findings; shared-root accounting prevents double counting; Prime promotion explicit. Live source exists (below, SNT-115); all four watchdog detector finding_types (`cssa.denial_streak`, `cssa.quota_exceeded_burst`, `cssa.approval_pending_burst`, `cssa.execution_failure_burst`) are wired into `promoteSingles()` (`src/intel/prime.ts`) as confidence-capped, recommend-only lone-signal incidents -- not yet a compound `CorrelationRule`, since none of the four independently corroborates another. |
...
- SNT-115 CSSA Evidence Adapter — **partial** (`cssaFindingToSourceFinding` now targets Forge-Agents' own canonical §4 `CloudSecurityFinding` shape and has a real, live-tested caller: `src/watchdog/` polls all three of DataForge's CSSA record families -- `decisions`, `authorizations`, `outcomes` -- and runs four deterministic detectors across them (denial streak, quota-exceeded burst, approval-pending burst, execution-failure burst), all shadow-only, all four wired to promote via Prime's `promoteSingles()`. `outcomes` carry no principal_id/tenant_id of their own (Forge-Agents §19); `AuthorizationIndex` (`src/watchdog/authorization_index.ts`) resolves a subject by joining on `authorization_id`, durable and pruned, never guessing at an unresolved outcome. A live production credential is still not wired -- this remains shadow/fixture-driven, not a live deployment)
