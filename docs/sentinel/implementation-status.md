...
| C — Correlation in shadow | done (MVP) | Watchdog findings adapt to source findings; shared-root accounting prevents double counting; Prime promotion explicit. Live source exists (below, SNT-115); both watchdog detector finding_types (`cssa.denial_streak`, `cssa.quota_exceeded_burst`) are now wired into `promoteSingles()` (`src/intel/prime.ts`) as confidence-capped, recommend-only lone-signal incidents -- not yet a compound `CorrelationRule`, since there is no second CSSA-sourced finding_type to correlate against. |
...
- SNT-115 CSSA Evidence Adapter — **partial** (`cssaFindingToSourceFinding` now targets Forge-Agents' own canonical §4 `CloudSecurityFinding` shape and has a real, live-tested caller: `src/watchdog/` polls DataForge's `decisions` family and runs two deterministic detectors -- denial streak, quota-exceeded burst -- decisions-only, shadow-only, per the 2026-09-20 ruling, and both now promote to a lone-signal incident via Prime's `promoteSingles()`; `authorizations`/`outcomes` and a live production credential are not yet wired)
